...
<head>
    ...
    <title>Bài 10 (Array): Danh lam thắng cảnh (Sắp xếp)</title>
    <style>
        body {
...
// Hàm so sánh để sắp xếp theo tên (Alphabet)
function compare_name($x, $y) {
    if ($x["ten"] == $y["ten"]) return 0;
    return ($x["ten"] < $y["ten"]) ? -1 : 1;
}

// Sắp xếp mảng sử dụng usort
usort($mang_dia_danh, "compare_name");

// Sau khi sắp xếp, tạo danh sách và nội dung
$danh_sach_link = "";
$nội_dung_chi_tiết = "";
...
foreach ($mang_dia_danh as $item) {
    ...
    // Giả lập đường dẫn hình ảnh (Sử dụng placeholder nếu không có file thực tế)
    // Tôi giữ lại logic hiển thị Nha Trang và Đà Lạt nếu bạn đã có file png tương ứng
